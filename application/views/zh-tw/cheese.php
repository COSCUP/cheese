...
<meta name="description" content="TOGETHER say Cheese!-- 請大家上傳最帥最正的社群或者歷屆 COSCUP 照片到 Picasa，並且加上 #coscup2u 標籤。透過 Google +1 button 投票，最高者將有意想不到的驚喜！" />
...
$.getJSON(base_url+"api",
        function(returnData){
			...
			if(returnData[0] == null){
				...
				getInvolved();
				$("#container").empty().append("<p>目前尚無照片，請參考「融入起司」，一起加入 COSCUP Cheese 吧！</p>");
				return;
			}
		  	data = returnData;
			$("#container").empty();
...
function arrangeEventTag(i, from, to, name, detail){
	//var tmp = new Date();
	//var d = new Date(tmp.getTime() + tmp.getTimezoneOffset() * 60000);
	var d = new Date();
...
function footerSwitcher(){
	if(footer_flag == 0){
		...
		$("#footer > font").fadeOut(1000, 'linear', function(){$("#footer").html('<font class="footer_text">COSCUP Cheese 2010 、 2011 近期上線，敬請期待！</font>');});
		footer_flag = 0;
	}
}
...
function selectTime(){
	...
	if(bounceTo_flag == 0){
		...
		$("#timeselect").append("<div class='day1 select_option' onclick='panToTime("+day1[0].to+");'>"+day1[0].name.replace('/', '<br />')+"</div>");
		for(var i=1; i<day1.length; i++){
		...
		$("#timeselect").append("<div class='select_option' style='text-align: center;width: 80px; margin-top: 20px; padding-left: 0px !important; clear: both;' onclick='closeTimeselect();'>關閉</div>");
...
<div id="getInvolvedInfo"><p>參與 <b>TOGETHER say Cheese !</b> 活動非常簡單，只要 <a href="http://picasaweb.google.com" target="_blank" style="color:#FFF">上傳最帥最正的社群或者歷屆 COSCUP 照片到 Picasa，並且加上 #coscup2u 標籤，並且將相簿設置「公開在網路上」</a>，然後這張照片就自然而然地與 COSCUP Cheese 混合了！</p><p>你也可以在活動期間 2011/8/8 - 8/14 間透過 Google +1 按鈕為其他的起司照片評分，得分最高者將有意想不到的驚喜！</p><p>*請注意，由於我們對照片數量做了上限限制，您上傳的照片有可能不會被顯示出來。</p><div class='select_option' style='clear: both;text-align: center;width: 80px; margin-top: 20px' onclick='closeGetInvolved();'>關閉</div></div>
